...
export function useClasses(
  ...
  options: Partial<UseQueryOptions<Classe[], Error>> = {}
) {
  ...
  return useQuery<Classe[], Error>({
    queryKey,
    queryFn: async () => {
      const response = await fetchData<ClasseListResponse>(
        "/classes",
        cleanParams
      );
...
export function useRequiredDocuments(
  ...
  options?: Partial<UseQueryOptions<RequiredDocument[]>>
) {
  return useQuery<RequiredDocument[]>({
    queryKey: ["required-docs", params.classeId],
    queryFn: async () => {
      return await fetchData<RequiredDocument[]>(
        `/classes/${params.classeId}/required-docs`
      );
    },
...
export function useClasse(classeId: number, full?: boolean) {
  return useQuery({
    queryFn: async () => {
      return await fetchOne<Classe>(
        full ? "/classes?full=true" : "/classes",
        String(classeId)
      );
    },
...
export function useClasseClassrooms(
  classeId: number,
  options: Partial<UseQueryOptions<Classroom[]>> = {}
) {
  return useQuery({
    queryFn: async () => {
